function passive_radar_app
    ...
    hFiles   = 360;                  % wysokości paneli po lewej
    hActions = 260;
    hStatus  = 200;

    % Pozycje paneli po lewej (od dołu: Status -> Actions -> Files)
    yStatus  = gap;
    yActions = yStatus + hStatus + gap;
    yFiles   = yActions + hActions + gap;

    % Prawa strona (wykres + CLim)
    rightX   = leftW + 2*gap;
    ...
    pActions = uipanel(fig,'Title','Operacje',...
        'Position',[gap yActions leftW hActions]);

    % Workspace / Status (bottom)
    pStatus = uipanel(fig,'Title','Status / Workspace',...
        'Position',[gap yStatus leftW hStatus]);
    ...
    for i=1:numel(fields)
        ...
        y = y-26;
        if y < 10, break; end % bezpieczeństwo przy bardzo małym oknie
    end

    %% === Action buttons ===
    uibutton(pActions,'Text','CAF - oryginalne','Position',[20 hActions-60 340 30],...
    ...
    function runCAF(mode)
        ...
        % wejście do filtrów: ostatni stan, chyba że 'orig'
        ...
        switch mode
            ...
            case 'close'
                surv_clean = clutter_removal(ref,surv_in,...
        ...
        % CAF
        [caf, delay_axis, doppler_axis] = CAF(ref,surv_clean,fs,fc,max_delay,doppler_bins,R);
    ...
    function runCLEAN()
        if ~isfield(data,'lastCAF')
            uialert(fig,'Najpierw uruchom CAF!','Błąd'); 
            return;
        end
        
        % CLEAN (estymacja celu bez zatwierdzania)
        ...
        if strcmp(choice,'Yes')
            ...
            title(ax,'CAF after CLEAN iteration');
            data.lastCAF = caf;
            disp('CLEN plotted');
        end
        updateStatus();
    ...
    function updateStatus()
        ...
        if isfield(data,'lastCAF')
            ...
            lines{end+1} = sprintf('CAF range (dB): [%.2f, %.2f]', ...
                                   min(data.lastCAF(:)), max(data.lastCAF(:)));
        else
        ...
        % Parametry
        lines{end+1} = '--- Params ---';
        lines{end+1} = sprintf('fs = %.2e Hz',data.params.fs);
        lines{end+1} = sprintf('fc = %.2e Hz',data.params.fc);
        lines{end+1} = sprintf('max_delay = %d',data.params.max_delay);
        lines{end+1} = sprintf('doppler_bins = %d',data.params.doppler_bins);
        lines{end+1} = sprintf('R = %d',data.params.R);
        lines{end+1} = sprintf('Close: Order=%d, Step=%.2e, Block=%d', ...
            data.params.filtOrder_close, data.params.stepSize_close, data.params.blockLength_close);
        lines{end+1} = sprintf('Wide : Order=%d, Step=%.2e, Block=%d', ...
            data.params.filtOrder_wide, data.params.stepSize_wide, data.params.blockLength_wide);
